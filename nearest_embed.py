...
class NearestEmbedFunc(Function):# 最近临嵌入
    """
    Input:
    ------
...
class NearestEmbedEMA(nn.Module):# 功能类似，额外使用了额外使用了指数移动平均来更新嵌入向量（self.weight是codebook）
    ...
    def forward(self, x):
        ...
        if self.training and not self.is_freeze:
            ...
            # )
            self.cluster_size.data.mul_(self.decay).add_(n_idx_choice, alpha=1 - self.decay)
